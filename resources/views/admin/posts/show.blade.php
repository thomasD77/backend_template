            ...
            <table class="table table-borderless">
                ...
                @foreach($post->comments->where('reply_id', null) as $comment)
                    <tr class="bg-body-dark mt-5">
                        <td class="d-none d-sm-table-cell"></td>
                        <td class="fs-sm text-muted">
                            <a class="fw-semibold" href="be_pages_generic_profile.php"><span class="text-muted me-4">Message from</span> {{ $comment->user->name }} on <span class="text-muted ms-4">{{ $comment->created_at->diffForHumans() }}</span></a>
                        </td>
                    </tr>
                    <tr>
                    ...
                    @foreach($post->comments->where('reply_id', $comment->id) as $commentReply)
                        <tr class="bg-body-light mt-5">
                            <td class="d-none d-sm-table-cell"></td>
                            <td class="fs-sm text-muted">
                                <a class="fw-semibold" href="be_pages_generic_profile.php"><span class="text-muted me-4">Reply from</span> {{ $commentReply->user->name }} on <span class="text-muted ms-4">{{ $commentReply->created_at->diffForHumans() }}</span></a>
                            </td>
                        </tr>
                        <tr>
                            <td class="d-none d-sm-table-cell text-center" style="width: 140px;">
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <div class="d-flex justify-content-end">
                                        ...
                                        <div class="form-group mr-1">
                                            {!! Form::button('<i class="far fa-comment me-1 opacity-50"></i> Reply',['type'=>'submit','class'=>'btn btn-alt-primary']) !!}
                                        </div>
                                    {!! Form::close() !!}
                                </td>
                            </tr>
